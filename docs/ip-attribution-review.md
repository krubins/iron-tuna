...
repository evidences a licence, permission, endorsement or affiliation from the
...
basis is "open licence", that is a public licence anyone may rely on, not a
...
"Player photos © NFL/ESPN". A copyright notice is not a licence: it names
...
1. **Leave as-is** with the corrected wording, and accept the risk that a rights
   ...
   widespread, but neither fact is a defence anyone has given in writing.
2. **Licence** an image set (Getty, AP, USA Today Sports Images and Imagn all
   sell fantasy-sports-scale licences).
3. **Substitute** — silhouettes, team colour blocks, or initials. Cheap, ugly,
   and completely safe.
...
4. Sleeper's written position, or the removal of the player map in favour of
   nflverse.
