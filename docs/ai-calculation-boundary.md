...
Neither coach holds a metric of its own: the DFS panel transports what `dfs-optimizer.js` and `buildDfsSlate`/`dfsMetrics` produced and nothing else. That includes the part of a partly-played slate that is already settled: a player row carries `gamePlayed`, the `actual` he scored (read through the page's own `actualOf`, so the row and the `final` chip beside it cannot disagree) and `actualFrom`, the slate's word for where that number came from. A banked `0` is a result and survives as one. Both reach the model through `/api/coach`, the server-side proxy, so no provider key reaches a browser.

The DFS panel carries the WHOLE eligible board, not only the rows the page printed. `slateBoard` is every priced player in the selected games that the solve could have used, grouped by position, one delimited line each (`name|team|opp|salary|proj|ceiling|own|value|status`) built by `coachBoard()` in `dfs.html`. It is transport, not a second metric: each cell is the same number `coachRow` would carry, rounded by `coachN` and nothing more, and a line that has no number for a column leaves it empty rather than filling it in. It exists because a reader asking for the best receiver at a given salary is asking about a player the recommendation never printed, and a coach that can only see the roster answers that by refusing. It carries exactly what the solve could use, which is `projected()` and not `onBoard`: the supplemental minimum-salary tier is on it, marked with a `basis` of `season` so a season-to-date average never reads as a forecast. A player who is not playing is off it, and so is one whose afternoon is BANKED — the builder's own rule is a scored actual, not `gamePlayed`, so a defense or a kicker whose game is final but whose number is still the projection stays in the pool, can be seated in the recommended roster, and is therefore on the index carrying a `basis` of `est`. The index tracking `gamePlayed` instead made it the one pool on the page missing a man the roster above it had already used, which is the failure mode the whole index exists to prevent. The prompt says the index is the complete pool, says which columns it carries and which it does not, and says that a name absent from it is out of the game pool, not playing, or already played — never that its number is unknown and could be estimated.

The budget the index is fitted to is derived from the prompt rather than written down beside it (`JSON_BUDGET = PROXY_SYSTEM_CAP - SYSTEM.length - PROMPT_JOIN - PROMPT_HEADROOM`). The proxy slices an oversized `system` instead of refusing it, so two hand-maintained constants that must sum to less than the cap are a silent truncation waiting to happen — and did become one the day a prompt edit and a budget rise landed the same afternoon.
