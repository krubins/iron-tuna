...
| 6 | Sleeper commercial dependency | **Blocking for production.** Non-commercial grant only; ships behind `FLAG_SLEEPER_SYNC` = off. Needs Sleeper's written licence (docs/data-sources.md R2/R7). |
...
| 22 | Known provider limitations | Sleeper: the licence; waiver status is derived ("dropped in the last two days"); future-week matchups depend on Sleeper exposing them. Yahoo: untested live; JSON is tolerant-parsed; the transactions resource is optional. ESPN: none. Manual: no opponent or matchups unless entered. |
