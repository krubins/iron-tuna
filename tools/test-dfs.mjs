...
console.log('\nthe DFS page explanations');
{
  ...
  ok('DraftKings terms expose hover definitions', page.includes('.df-term:hover::after') && page.includes('data-tip="DraftKings&rsquo; weekly salary-cap game.') && page.includes('data-tip="Draft the roster once.') && page.includes('data-tip="A cash contest where roughly half the field') && page.includes('data-tip="You may enter only one lineup') && page.includes('data-tip="A tournament that allows the same user to enter more than one lineup') && page.includes('data-tip="Guaranteed Prize Pool.'));
  ok('the primary Classic contest cards use DraftKings lobby terminology', page.includes('>50/50</span> &amp; <span class="df-term"') && page.includes('>Double Ups</span>') && page.includes('>Tournament</span> &middot; <span class="df-term"') && page.includes('>Single Entry</span>') && page.includes('>Multi-Entry Tournament</span>') && !/<button class="df-shape"[^>]*data-contest="3max"/.test(page));
  ok('the DraftKings Classic translation explains the second step', page.includes('Step 2, after choosing <span class="df-term"') && page.includes('>Classic</span>') && page.includes('>50/50</span>') && page.includes('>Single Entry</span>') && page.includes('>Multi-Entry Tournament</span>'));
  ok('Classic requires an exact game pool as step three', page.includes('Step 3 · Required') && page.includes('Select the Games in Your DraftKings Contest') && page.includes('Choose at least one game before Iron Tuna builds a lineup.'));
  ok('game selection actually filters the optimizer player pool', page.includes('function filteredSlate()') && page.includes('selectedGames[playerGameKey(p)]') && page.includes('var view = filteredSlate();') && page.includes('var players = view.players.filter'));
  ok('all DFS boards use the selected game pool', page.includes('dashboard(view); envTable(view); values(view); pool(view); stacks(view); tdBoard(view);'));
  ok('no game selection blocks lineup construction', page.includes('function showGameScopeRequired()') && page.includes('Iron Tuna will not build a lineup until the eligible game pool is defined.'));
  ok('game scope offers exact games plus time-window conveniences when kickoff data is present', page.includes('data-game-preset="all"') && page.includes('1 PM ET games') && page.includes('Late afternoon') && page.includes('Primetime'));
  ...
  ok('GPP is defined without treating it as an entry-limit label', page.includes('>GPP</span></b> means Guaranteed Prize Pool') && page.includes('GPP does not tell you whether the contest is single-entry or multi-entry'));
