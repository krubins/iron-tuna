...
function applySitemap() {
  ...
  // ── the ~400 player cards ──────────────────────────────────────────────────
  // /player/<slug> is one shell answering at every player on the board, exactly
  // as /analysts/<id> is one shell answering at eight. _worker.js rewrites the
  // head and pre-renders the card for each, which is what makes them indexable;
  // this is what makes them findable.
  //
  // A MANAGED BLOCK, not appended lines. The hand-curated entries above are
  // never rewritten because a human chose their priorities; these are chosen by
  // nobody and are regenerated wholesale on every projections update, when
  // players arrive, change clubs and fall off. Keeping them between markers is
  // what lets this tool remove a player it no longer prices instead of leaving
  // a 404 advertised forever.
  //
  // The roster is read from /player-search.js, the same generated block the
  // worker parses — not from a list kept here, which would be a third copy.
  out = applyPlayerUrls(out);

  write(file, out, before);
}

const PL_OPEN = '  <!-- players:begin — generated by tools/build-seo.mjs from player-search.js -->';
const PL_CLOSE = '  <!-- players:end -->';

// The slugs, straight out of the block build-front.mjs writes.
function playerSlugs() {
  const m = /var INDEX_RAW = "([\s\S]*?)";/.exec(read('player-search.js'));
  if (!m) { console.error('  WARN player-search.js: no INDEX_RAW, no player URLs in the sitemap'); return []; }
  const out = [];
  for (const line of m[1].split('\\n')) {
    const f = line.split('|');
    if (f.length < 4 || !f[0] || !f[1] || !f[3]) continue;
    out.push(f[0]);
  }
  return [...new Set(out)].sort();
}

function applyPlayerUrls(xml) {
  const slugs = playerSlugs();
  if (!slugs.length) return xml;
  ...
  const re = new RegExp(PL_OPEN.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[\\s\\S]*?' + PL_CLOSE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
