...
console.log('\nthe migration');
{
  ...
  ok('every retired kind is gone from the calendar', ['mnf-preview', 'team-recaps', 'mnf-breakdown', 'what-they-arent-telling-you', 'opportunity-report', 'rankings-update', 'final-read', 'tnf-aftermath', 'weekend-game-plan', 'what-changed-today', 'snf-what-we-learned'].every(k => !H.CONTENT_KINDS[k] && H.LEGACY_CONTENT[k]));
  ...
  ok('Monday has one 6 AM package, not two', H.CONTENT_KINDS['early-rankings'].title === 'Monday Morning Brief' && !H.CONTENT_KINDS['mnf-preview'] && H.CONTENT_KINDS['early-rankings'].absorbs.includes('mnf-preview'));
  ok('discretionary publication is staggered through the day', H.CONTENT_KINDS['quarterback-monday'].hour === 12 && H.CONTENT_KINDS['tailback-tuesday'].hour === 13 && H.CONTENT_KINDS['wideout-wednesday'].hour === 13 && H.CONTENT_KINDS.underrated.hour === 10 && H.CONTENT_KINDS['trade-desk'].hour === 13 && H.CONTENT_KINDS['tight-end-thursday'].hour === 16 && H.CONTENT_KINDS['weekend-preview'].hour === 11 && H.CONTENT_KINDS['kickers-defenses'].hour === 14);
  ok('Saturday Market Movers is on the calendar at 11:30 AM', H.CONTENT_KINDS['market-movers'].day === 'Sat' && H.CONTENT_KINDS['market-movers'].hour === 11 && H.CONTENT_KINDS['market-movers'].minute === 30);
  const prewrite = Object.values(H.CONTENT_KINDS).filter(k => k.generateHour != null);
  ok('all prewritten packages have overnight generation slots', prewrite.length >= 10 && prewrite.every(k => k.generateHour >= 0 && k.generateHour < 6));
  ok('time-sensitive Sunday stories are not prewritten', H.CONTENT_KINDS['last-minute-intel'].generateHour == null && H.CONTENT_KINDS['what-sunday-taught-us'].generateHour == null);
