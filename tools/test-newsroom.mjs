...
console.log('\nthe fact check');
{
  ...
  // THE BACKFILL IS A REWRITE, NOT A RE-REPORT. It is handed a published
  // piece's own DFS sections and asked for a headline over them; it must never
  // reach for the packet's facts again, because the week it describes has
  // moved on. The prompt is the contract, so the prompt is what is asserted.
  ok('the backfill is told to use only the published sections',
     /only what the sections below already say/i.test(H.DFS_BACKFILL_SYSTEM)
     && /introduce no player, team or number/i.test(H.DFS_BACKFILL_SYSTEM));
  ok('and told what a DFS headline is not', /never a waiver, trade or season-long roster call/i.test(H.DFS_BACKFILL_SYSTEM));
}
