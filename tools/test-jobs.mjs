...
const runs = [];
const JOB_FNS = Object.fromEntries(['schedule-refresh', 'odds-refresh', 'availability-refresh', 'market-snapshot', 'usage-refresh', 'usage-prior-refresh', 'dfs-refresh', 'depth-charts', 'ros-snapshot', 'calls-grade', 'news-scan', 'snapshot-prune', 'analytics-prune', 'job-prune', 'content-tick', 'league-sync'].map(j => [j, async () => ({ ok: true })]));
...
console.log('\nthe worker source');
{
  ...
  ok('Thursday 6, 7, 8, 9 AM: TNF preview, Underrated, Trade Desk, Tight End Thursday', hour('tnf-preview') === 'Thu 6' && hour('underrated') === 'Thu 7' && hour('trade-desk') === 'Thu 8' && hour('tight-end-thursday') === 'Thu 9');
  ok('Friday 6, 7, 8 AM: TNF What Matters, Weekend Preview, Kickers & Defenses', hour('tnf-what-matters') === 'Fri 6' && hour('weekend-preview') === 'Fri 7' && hour('kickers-defenses') === 'Fri 8');
  ok('no retired kind is on the calendar', ['what-changed-today', 'snf-what-we-learned', 'team-recaps', 'mnf-breakdown', 'what-they-arent-telling-you', 'opportunity-report', 'rankings-update', 'final-read', 'tnf-aftermath', 'weekend-game-plan'].every(k => !new RegExp("'" + k + "': \\{").test(kinds)));
