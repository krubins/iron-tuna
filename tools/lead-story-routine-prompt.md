...
1. Read `PROJECTIONS` and `VEGAS_WEIGHT` out of `_worker.js`. If the connector fetch fails or the slot is running short, `tools/live-board.mjs` in the repo does this same build and lifts every constant out of `_worker.js` at run time, so it cannot go stale: `board(overlayPath)` returns the served board and `board(null, false)` the committed one. It is a **fallback**, not a replacement — the deployed Worker is still the authoritative source, and a repo checkout can lag it.
...
- Score one heavily-lined player both ways and confirm the numbers differ. Chuba Hubbard was RB35 at $2 committed and RB29 at $3 blended on the August 31 board. Those exact ranks and prices move whenever the projection set is updated, as it was on 2026-08-30, so **the pass condition is that your two boards DISAGREE on him, not that they reproduce the figures printed here.** If your two boards agree on him, your blend is not running, and every price you are about to print is off the wrong board.

**Never validate your board against `DEFAULT_BOARD_RAW` in `it-league.js`. A match there is never a pass.** That block is generated from the committed array and has never seen an odds refresh, so it will agree with an unblended board perfectly and tell you nothing. A run on 2026-08-24 reported "343 players overlap, 332 match to within 0.15 points, exact on all four" against it and published six figures the served board contradicts, including Jonathan Taylor at "RB4 and $55" where the sheet says RB5 and $50. Two wrong boards agreeing is not a check. The only thing that confirms a served figure is right is your blended build.

**Its one legitimate use is the opposite, and it is now required.** Because that block IS the committed board, computed from the same worker by a different tool, a figure you are presenting as the SERVED board that matches it exactly is almost certainly a committed figure printed in a served column. There, the match is the ALARM, not the pass. That is the BOARD ATTRIBUTION CHECK below, and it is the only context in which you may compare anything to `DEFAULT_BOARD_RAW`.
...
**If you reprice a player by changing his projection, re-rank the position with him taken out of his old slot.** Say a back is RB6 and you ask what one missed game costs him. Scale his line, then rebuild the ladder *without* his old entry and insert the new score — every back below RB6 moves up one when he leaves. Reading the adjusted score against the standing list is off by exactly one slot, and one slot is one curve step, which is real money. A run on 2026-08-29 did that and published "$42" in its own headline for a player the corrected ladder prices at $43.
...
**BOARD ATTRIBUTION CHECK. Do this for every figure, and do it before you insert.** You will have TWO correct boards in front of you — the served board (odds blended) and the committed board (`board(null, false)`, the raw `PROJECTIONS` array). Both are right. Reading a number off the wrong one is now the single most common way this column publishes something the reader's sheet contradicts, and it has shipped twice in two days:

- **2026-09-03, row 73.** Tony Pollard printed as "$5, RB28". The sheet had him at RB29 and $3 on every board that run could have built. RB28 at $5 was the slot ABOVE him, belonging to Jaylen Warren. The run had read a rank slot's price and attached it to the player one row down.
- **2026-09-04, row 77.** Cam Skattebo printed as "$15, RB18" in a table headed "Iron Tuna board, September 4" — the served board. $15 and RB18 are his COMMITTED figures; served, he is $13 and RB19. The same error then produced a rank-move cell reading "RB18 either way, worth $0" when the true move is RB18 to RB19, worth -$2.

Both runs rebuilt the board correctly. Both then reported that every figure had been "checked against the board built here" — **a build checked against itself, which cannot see a number taken off the wrong board.** So:

1. **For every price and rank you print, look it up in BOTH boards and write down both answers.** Not the one you expect; both.
2. **Say in the story which board each printed figure is.** A column header naming a date means the served board. If a cell in that column carries a committed figure, that is a defect, not a nuance.
3. **Then check each figure you are presenting as served against `DEFAULT_BOARD_RAW` in `it-league.js`.** It is `Name|POS|points`, already sorted by points within position, so a player's index within his position block is his committed rank minus one, and the curve value at that slot is his committed price. Zero-point players are dropped from it, so absence there is not a signal. If a figure you are calling served matches that committed rank or price exactly, you have almost certainly printed the committed number: go back to your served build and fix it, or cut the figure. (A player the odds do not move will legitimately match — confirm that from your own two boards, say so in the story, and note it in the method line rather than leaving it implied.)

   This is the one comparison that is independent of your own build. `it-league.js` is generated from the same worker by a different tool (`tools/build-default-board.mjs`), so it cannot repeat a mistake your build just made. Everything else in your verification is your build checking itself.
4. **Every "rank move", "worth $N", or "up from" cell must equal served minus committed for that player**, recomputed, not carried over. "No move" and "either way" are claims about both boards and must be true of both.
5. **Verify by player name, never by slot.** Read the row for the player you are naming. Do not read a curve slot's price and assume it belongs to whoever you think sits there — that is exactly what went wrong on 2026-09-03.

State in the method line that you ran this check and what it returned. If a figure fails it and you cannot resolve which board it belongs to, cut the figure rather than publishing it.

**That check covers derived numbers too, not only prices and ranks.** Any figure you compute from the board or the odds — a difference, a ratio, a count, a span, a "more than" or "twice as much as" — gets recomputed and checked the same way. A run on 2026-08-27 printed "five more scores on the ground" for a gap of 4.2. On 2026-08-30 a sweep found a story claiming one quarterback trailed another by 1.5 points a game when the board said 1.6, and by 1.9 when it said 1.8; nobody had reported either, and checking prices alone would not have found them.
