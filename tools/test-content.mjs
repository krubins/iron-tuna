...
console.log('\nthe validator');
{
  ...
  const jh = H._finishBrief({ players: [{ name: 'Justin Herbert', proj: 18.2 }] });
  const nl = H.validateDraft(JSON.stringify({ a: 'Start Herbert.\nVega ranks him QB4. Lock Herbert in.' }), jh.allowed);
  ok('an escaped newline in the JSON body is a sentence end, and an imperative is not a first name', nl.ok, JSON.stringify(nl));
  ...
  const shown = H.validateDraft('Lawrence at $5,800 saves $1,200 against Allen at $7,000; Nix at $4,700 and Darnold at $4,000 are a 700-dollar difference.', pay.allowed);
  ok('money is arithmetic on money when the draft shows its work: both salaries and their difference pass', shown.ok, JSON.stringify(shown));
