...
- `composeTweet(insight)` builds: `🏈 {title}` + a blank line + **"Run your draft using the strongest, most advanced AI in fantasy football."** + the insight's canonical URL + format-specific hashtags (`#FantasyFootball #AuctionDraft #FFDraft` or `#FantasyFootball #SnakeDraft #FFDraft`). Longest current title produces a ~274-char tweet (under the 280 limit); 2 of 140 titles are long enough to get auto-truncated with an ellipsis.
