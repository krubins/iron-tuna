...
Posts to **@irontunafantasy** every **weekday at 14:00 UTC** (10am ET / 9am during EST) — **exactly one thread per day** since the July 2026 reach rework: Monday an auction insight, Tuesday a snake feature/discount post, Wednesday auction strategy/Value Coach/comparison/mock promos, Thursday a snake insight, Friday best ball. Runs via a Cloudflare Worker **Cron Trigger**, no external scheduler needed.

**Reach rework rationale (July 2026):** the original cadence (two insight threads + a Tue-Fri bonus, 10-13 threads/week) got ~zero views — a zero-follower account posting high-volume, template-identical, link-bearing threads is exactly what X's algorithm buries as automation. So: (1) volume cut to one post/weekday; (2) **X posts are link-free by default** — replies end with a `🔗 Link in bio.` marker instead of a URL (X measurably suppresses posts with external links, and they cost 13x on pay-per-use pricing) — with Wednesday's conversion post the one weekly exception that keeps a real URL; (3) hashtags cut from three to one (`#FantasyFootball`, `#BestBall` on Friday) — tag stuffing reads as spam and drives no discovery. **⚠️ Manual step this depends on: the irontuna.com link must be put in the @irontunafantasy profile bio** (and kept there), or "Link in bio" points at nothing. The Threads mirror is unaffected: `linkifyForThreads` swaps the marker back to the real URL there, since Threads doesn't suppress links and charges nothing per post. Growth beyond this still requires manual engagement from the account (replies to big fantasy accounts with real data takes); the auto-poster is the content backbone, not the growth engine.
...
- `composeThread(insight)` builds a **2-tweet thread**, not a single tweet, so real analysis fits: tweet 1 is `🏈 {title}` + `📊 Projected effect: {stat}` + `💡 {play takeaway}` — the stat line is included whenever the full trio fits in 280 (198/210 currently) and silently dropped otherwise, never truncated mid-number; the title takes whatever room the play line doesn't need and every current title+play pair fits untruncated; tweet 2 is a reply opening **"Insight N of 5 in our <Month D> <format> drop."** (N parsed from the insight id; the date+format descriptor is what keeps every reply's text unique now that replies carry no URL — X permanently rejects exact-duplicate tweets, and a bare "Insight 1 of 5" would collide across every drop page) + `X_TAGLINE` + the `🔗 Link in bio.` marker (see the link-free policy above) + a single hashtag. Max observed lengths re-verified programmatically across all 256 composable threads after the reach rework: 280 (hook) / 207 (reply), both within the limit.
- `runXAutoPost(env)` posts exactly **one thread per run**: it resolves the UTC weekday's slot from `X_DAY_SLOTS` and picks that slot's next item by rotation index (`SELECT COUNT(*) FROM x_posts WHERE format=?`), so each pool cycles fully before any repeat. Only insights whose page `date` has already unlocked (≤ today) are eligible. Cross-format duplicate headlines (the drop pages occasionally republish the same take in two formats) are handled by the text-hash duplicate guard below, which walks past any hook already posted.
...
- `scheduled(event, env, ctx)` at the bottom of `_worker.js` calls `runXAutoPost(env)`, which posts the day's single thread and logs each attempt to a D1 table `x_posts (insight_id, format, tweet_id, ok, posted_at)` — `tweet_id` is a comma-joined pair (hook,reply) when both post successfully.
- **Day slots:** `X_DAY_SLOTS` maps UTC day-of-week → `{pool|dynamicPool, compose, format}` for all five weekdays. Mon/Thu are the auction/snake insight-thread slots (`composeThread`); Tue/Wed/Fri below were the old "bonus" pools and are now those days' only post.
...
- **Manual trigger for testing:** `GET /api/admin/x-post-now?key=<LEADS_EXPORT_KEY>` runs the same `runXAutoPost` on demand and returns the composed thread text + tweet IDs — use this to verify before waiting for the next cron tick. Add `&day=<1-5>` (1=Mon..5=Fri, matches cron numbering) to force that weekday's slot on an off-day test run.
...
**How it works:**
- `postAndLogThreads(env, format, id, tweets, imagePath)` is called right after each successful (or skipped) X post, using the same `pick.tweets` array X just composed, with one difference since the reach rework: `linkifyForThreads` swaps X's `🔗 Link in bio.` marker for the item's real URL, since Threads doesn't suppress link posts and has no per-post link charge. If X's pool is exhausted for a slot (`no_insight_available`), Threads is skipped too for that slot, same as X.
