...
**What a reader can now do.** Type a name in the white ribbon on the front page,
pick a face out of the list, and land on that player's card at
`/player/<slug>`. The ribbon is `position: sticky`, so the box is on screen the
whole way down the page rather than only at the top. The card carries the same
box, because the second thing a reader does on a player card is look up another
player.

### The pieces

- **`player-search.js`** — the lookup index and the widget that reads it, as one
  plain file loaded by `front.html` and `player.html` alike. It carries
  **identity only**: `slug|Name|TEAM|POS|espnId|nflId`, one line per player, ~28
  KB for 407 rows. The block between the `generated by tools/build-front.mjs`
  sentinels is generated, exactly like `DEFAULT_BOARD_RAW` in `/it-league.js`.
...
- `tools/test-player-card.mjs` — index coverage against `PROJECTIONS` (by name,
  ...
  place in the sticky ribbon, and that every player a published call names *and
  the app prices* has a card.
