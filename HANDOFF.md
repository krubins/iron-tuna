...
- `composeThread(insight)` builds a **2-tweet thread**, not a single tweet, so real analysis fits: tweet 1 is `🏈 {title}` + `📊 Projected effect: {stat}` + `💡 {play takeaway}` — the stat line is included whenever the full trio fits in 280 (198/210 currently) and silently dropped otherwise, never truncated mid-number; the title takes whatever room the play line doesn't need and every current title+play pair fits untruncated; tweet 2 is a reply opening **"Insight N of 5 in the Jul 4, 2026 drop."** (N parsed from the insight id, the date rendered by `dropLabel()` from the insight's `date` — sells the click by promising four more takes AND makes each reply's text unique, which matters because X permanently rejects exact-duplicate tweets and, with no URL in the X copy, index + drop date + format hashtags carry uniqueness on their own; the year is included so future seasons can't collide) + `X_TAGLINE` + the insight's canonical URL + format-specific hashtags (`#FantasyFootball #AuctionDraft #FFDraft` or `#FantasyFootball #SnakeDraft #FFDraft`). **The URL line never reaches X** — `stripSiteLinks()` in `postAndLog()` removes any line containing an irontuna.com link before posting (Aug 2026 decision: no site links in X posts; it's also the difference between X's $0.20 link-post and $0.015 plain-post rates) — but the compose functions still emit it so the Threads mirror (§11) keeps the link. Max X tweet length across the full current pool: 279 chars, all under 280, no duplicate X replies (re-verified programmatically across all 250 composable threads, Aug 2026 link-removal change).
...
**How it works:**
- `postAndLogThreads(env, format, id, tweets, imagePath)` is called right after each successful (or skipped) X post, using the exact same `pick.tweets` array X's run composed — so the two platforms carry the same message the same day. One deliberate divergence (Aug 2026): Threads receives the tweets **before** `stripSiteLinks()` runs, so Threads replies still include the irontuna.com link that X posts omit (Threads' API is free and doesn't charge per link). If X's pool is exhausted for a slot (`no_insight_available`), Threads is skipped too for that slot, same as X.
