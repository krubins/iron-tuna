...
- **Tue-Fri bonus third post:** `X_BONUS_DAY_POOLS` maps UTC day-of-week → `{pool, compose, format}` (or `{dynamicPool, compose, format}` for Friday — see below).
  ...
  - **Tuesday and Thursday** (days 2 and 4) both use `X_SNAKE_BONUS_POOL`, a 2-way interleave of `X_SNAKE_FEATURE_POSTS` (11 tweets: 8 promoting the live survival-odds / "Will he be available?" feature — knowing who'll last to round 5 changes what you take in round 2 — plus 3 promoting the "See the future" ideal-roster-construction plan, angled on "picking the right players won't help you if you have the wrong construction", grounded in the landing page's Snake Draft bullet and `buildSnakePlan`'s ideal-by-round output) and `X_COMPARISON_SNAKE_POSTS` (4 tweets contrasting the same live survival-odds/Value Coach/pricing angle against free static draft rooms (Sleeper/ESPN) and paid subscription rankers). Both days share one rotation counter (format `snakefeature`), so Tue and Thu together cycle through all 15 without repeating.
