...
CBS browser connector 0.2.0: the token-declaration approach failed on the live CBS league. The extension now reads whitelisted settings/scoring, roster-grid team names and every team roster through same-origin requests in the signed-in CBS tab, then posts a bounded snapshot to the existing connect route as provider cbs_browser. No CBS credentials leave the browser. The existing API-token adapter and encryption are retained separately; as of 2026-09-16 it sends the token as CBS's `access_token` query parameter (and the Authorization header), treats a sign-in redirect as a refused token instead of a generic failure, and every failure shows a redacted diagnostic (resource, HTTP status, redirect host/path) on the form and in the league's last error, so a live failure says what CBS answered. Browser leagues never run in the scheduled sync job and expose no next automatic refresh time.
