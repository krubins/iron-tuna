...
**Superflex and 2-QB get their own curve.** `SUPERFLEX_QB_CURVE` is a real superflex market, not the old 1-QB row kept around: 32 entries (a 12-team superflex rosters 24 starting QBs; the old 16-entry row dumped everyone past QB16 on the min bid, pricing a required starter like a handcuff), and QB1 is the most expensive player on the board, which is what a room that must start two quarterbacks actually does.

It is calibrated the same way as the 1-QB row — against VALUE, which already carries the superflex scarcity through the replacement level, so the curve only has to add the room's premium on top. On the real board that lands **QB top-14 PROJ/VALUE at 1.21x against the 1-QB board's 1.19x**: the same premium, because the premium is a fact about rooms and the scarcity is already in VALUE. Cross-checked independently by budget share — the 24 starting QBs ask $750 of a 12-team $2400 pool (31%) against the $645 (27%) VALUE says they are worth.

**Measure the premium as QB against its OWN VALUE, never as QB's ratio relative to RB/WR/TE.** The relative version looks like the more robust metric and is not: superflex moves a quarter of the pool onto quarterbacks, so every other position's PROJ/VALUE falls with it (RB 0.88x, WR 0.98x, TE 0.86x on the superflex board) and the relative figure swings on the reallocation rather than on the premium. Calibrating against it drove the curve a third too hot before the error showed up.

**The tail is floored at full value, and that is not a mistake.** In a 1-QB room the tail is optional — nobody has to own QB20, so the market pays UNDER his value and the 1-QB curve decays past 1.0x. Superflex inverts it: 24 quarterbacks must be rostered, so the last starters get bid up to at least what they are worth. The multiplier decays through the tier where a manager still has a choice and then floors at 1.0x.

`it-league.js` carries the superflex row too, gated on the same `qbPremium` test read off the reader's saved league, so a superflex reader is not shown 1-QB quarterback prices on the news pages. That is a **fourth** hand-synced copy with no build step; `tools/test-it-league.mjs` lifts both and fails on drift. `_worker.js`'s `COLUMN_CURVE` deliberately does NOT carry it — the Vegas column prices the site's default league, which is 1-QB, not any individual reader's.
